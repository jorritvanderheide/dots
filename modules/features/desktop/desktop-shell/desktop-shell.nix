    ...
    in
    {
      options.my.desktop-shell = {
        ...
        monitor = lib.mkOption {
          ...
          example = "DP-1";
          description = ''
            Name of the monitor the top bar should appear on (e.g. niri output name).
            When empty, the top bar follows the focused output.
          '';
        };
      };
        ...
        home-manager.sharedModules = [
          {
            ...
            # Environment values below. Tied to graphical-session.target so it
            # starts with the compositor session and stops when the session ends.
